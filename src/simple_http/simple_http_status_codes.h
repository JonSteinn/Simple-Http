...
struct ____mzQWuRLpvb____HdsFYMewNA____ {
    /**
     ...
     * 100 Continue status code in response before sending the body. If the client
     * receives an error code such as 403 (Forbidden) or 405 (Method Not Allowed)
     * then it shouldn't send the request's body. The response 417 Expectation
     ...
     */
    const int32_t ALREADY_REPORTED;
     ...
     */
    const int32_t MULTIPLE_CHOICES;

    /**
     * This and all future requests should be directed to the given URI.
     */
    const int32_t MOVED_PERMANENTLY;
     ...
     * contradicting the standard. The HTTP/1.0 specification (RFC 1945) required
     * the client to perform a temporary redirect (the original describing phrase
     * was "Moved Temporarily"), but popular browsers implemented 302 with the
     ...
     */
    const int32_t TEMPORARY_REDIRECT;
     ...
     */
    const int32_t PERMANENT_REDIRECT;
     ...
     */
    const int32_t PAYMENT_REQUIRED;
     ...
     * of some sort.
     */
    const int32_t FORBIDDEN;

    /**
     * The requested resource could not be found but may be available in the
     * future. Subsequent requests by the client are permissible.
     */
    const int32_t NOT_FOUND;
     ...
     */
    const int32_t METHOD_NOT_ALLOWED;
     ...
     * negotiation.
     */
    const int32_t NOT_ACCEPTABLE;

    /**
     * The client must first authenticate itself with the proxy.
     */
    const int32_t PROXY_AUTHENTICATION_REQUIRED;
     ...
     */
    const int32_t UNSUPPORTED_MEDIA_TYPE;
     ...
     */
    const int32_t RANGE_NOT_SATISFIABLE;

    /**
     * The server cannot meet the requirements of the Expect request-header field.
     */
    const int32_t EXPECTATION_FAILED;
     ...
     */
    const int32_t IM_A_TEAPOT;

    /**
     * The request was directed at a server that is not able to produce a response
     * (for example because of connection reuse).
     */
    const int32_t MISDIRECTED_REQUEST;

    /**
     * The request was well-formed but was unable to be followed due to semantic
     * errors.
     */
    const int32_t UNPROCESSABLE_ENTITY;
     ...
     */
    const int32_t REQUEST_HEADER_FIELDS_TOO_LARGE;
     ...
     */
    const int32_t UNAVAILABLE_FOR_LEGAL_REASONS;

    /**
     * A generic error message, given when an unexpected condition was encountered
     * and no more specific message is suitable.
     */
    const int32_t INTERNAL_SERVER_ERROR;
     ...
     */
    const int32_t NOT_IMPLEMENTED;

    /**
     * The server was acting as a gateway or proxy and received an invalid
     * response from the upstream server.
     */
    const int32_t BAD_GATEWAY;

    /**
     * The server is currently unavailable (because it is overloaded or down for
     * maintenance). Generally, this is a temporary state.
     */
    const int32_t SERVICE_UNAVAILABLE;

    /**
     * The server was acting as a gateway or proxy and did not receive a timely
     * response from the upstream server.
     */
    const int32_t GATEWAY_TIMEOUT;

    /**
     * The server does not support the HTTP protocol version used in the request.
     */
    const int32_t HTTP_VERSION_NOT_SUPPORTED;

    /**
     * Transparent content negotiation for the request results in a circular
     * reference.
     */
    const int32_t VARIANT_ALSO_NEGOTIATES;

    /**
     * The server is unable to store the representation needed to complete the
     * request.
     */
    const int32_t INSUFFICIENT_STORAGE;

    /**
     * The server detected an infinite loop while processing the request (sent in
     * lieu of 208 Already Reported).
     */
    const int32_t LOOP_DETECTED;

    /**
     * Further extensions to the request are required for the server to fulfill
     * it.
     */
    const int32_t NOT_EXTENDED;
     ...
     */
    const int32_t NETWORK_AUTHENTICATION_REQUIRED;
};
